...
class SerialStreamerProvider with ChangeNotifier {
  ...
  SerialStreamerProvider() {
    serialSearchTimer =
        Timer.periodic(Duration(seconds: 1), serialSearchTimerCallback);
  }
        ...
        try {
          ...
          _incomingDataStream!.listen((data) {
            ...
            for (var element in data as Uint8List) {
              ...
              var _hex = element.toRadixString(16).toUpperCase();
              // hex += element.toRadixString(16).toUpperCase() + " ";
              hex += _hex.padLeft(2, "0") + " ";
            }
            ...
            dataList.clear();
            for (var i = 0; i < ascii_list.length; i++) {
              if (_ascii) {
                dataList.add(ascii_list[i]);
              }
              if (_binary) {
                // dataList.clear();
                dataList.add(binary_list[i]);
              }
              if (_decimal) {
                // dataList.clear();
                dataList.add(decimal_list[i]);
              }
              if (_hex) {
                // dataList.clear();
                dataList.add(hex_list[i]);
              }
            }
